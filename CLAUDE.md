...
**As of 2026-09-23, prod and staging are fully separate deployments, each with its own
dedicated Postgres database** — see "Prod/dev split" below. Every route/table described
below is a single, plain path/name; there's no more `/dev/`-prefixed duplicate.
...
- `GET /g/<id>` — renders `static/index.html` per request, with `<title>`/`og:*`/
  `twitter:*` tags swapped in for that specific game (escaped — see below). This **is**
  the real app, not a redirect or a stripped viewer; the frontend's `init()` detects
  this path and loads just that one game.
- `GET /g/<id>/preview.png` — a board-position PNG, rendered on demand (not stored)
  from the game's PGN, halfway through the mainline.

## Static frontend + single-game sharing

**Serving** (new `static/` folder, moved here from the old `chess-library` repo):
- `GET /` → `static/index.html` — byte-identical to what GitHub Pages used to serve.
  `Cache-Control: no-cache` so a push goes live immediately, matching the old behavior.
  Just this one file now — see "One frontend file, not two" below.
...
**`share_url_for()`/`request_origin()` derive the share link's domain from the incoming
request** (`X-Forwarded-Proto` header + `request.host`, defaulting to `https`), not a
hardcoded constant. This used to be a hardcoded `SHARE_HOST = "https://library.chessscenes
.com"` — harmless for `web` (which really does serve that domain) but actively wrong for
`chess-library-api-staging`, whose own separate database means a share created there
produces a row that only *that* service's `/g/<id>` route can read; a hardcoded prod
domain in the link would 404 (right domain, wrong database, once the two services stopped
sharing one Postgres — see "Prod/dev split"). Deriving it from the request instead means
every deployment's share links are correct for wherever they were actually created,
...
- `sessions` — `(token TEXT PRIMARY KEY, user_id INTEGER, created_at TIMESTAMPTZ
  DEFAULT now())`.
- A `libraries` row can optionally carry a `user_id` (nullable, `UNIQUE`) linking it to
  an account. Existing anonymous rows have `user_id IS NULL`, which Postgres allows any
  number of under a unique index/constraint — they don't conflict with each other or
  with the per-account uniqueness rule.
...
- `share_events` — `(id SERIAL PRIMARY KEY, ip TEXT, created_at TIMESTAMPTZ DEFAULT
  ...
  share endpoint's rate limit.
- **No more `_dev`-suffixed tables** — see "Prod/dev split" below. `web`'s own Postgres
  still has orphaned `libraries_dev`/`users_dev`/`sessions_dev` tables left over from
  before the split below was introduced; nothing reads or writes them, left in place
  rather than dropped since dropping tables isn't a call to make silently.

## Prod/dev split

**As of 2026-09-23, prod (`web`) and staging (`chess-library-api-staging`) are fully
separate Railway services, each with its own dedicated Postgres database** — see the
"Staging environment" section below for the full history of how this replaced an
earlier, riskier design. Practically, this means:
- Every route in this file is a single plain path — no `/dev/`-prefixed page route, no
  `/api/dev/...` route prefix, no `env` parameter threaded through the handler
  functions in `app.py`. "Which environment" is entirely a function of which
  `DATABASE_URL` a given process was started with.
- A push to `main` only ever touches `web`; a push to `staging` only ever touches
  `chess-library-api-staging`. There's no longer a single shared backend process that
  both prod and staging frontends depend on.
- Staging's session tokens/library rows live in a completely different database than
  prod's — not just a different table within the same one — so there's no way for
  staging traffic to touch real user data even by accident.

## Environment variables (set on Railway)

- `DATABASE_URL` — each service's own dedicated Postgres, auto-linked via a variable
  reference (`${{Postgres.DATABASE_URL}}` on `web`, `${{Postgres-wpZy.DATABASE_URL}}` on
  staging) — never hardcoded, and **never the same reference on both services** (that
  was the old, since-replaced design — see "Staging environment" below).
- `ALLOWED_ORIGIN` — **comma-separated list** of allowed CORS origins (parsed via
  ...
  matters for any *other* origin calling this API, not the app's own traffic. `web`'s
  value is currently `https://harman28.github.io,https://library.chessscenes.com` — the
  `github.io` entry is now vestigial (that site no longer exists post-consolidation) and
  can be dropped whenever, no urgency. Staging's own value additionally includes
  `https://library-staging.up.railway.app`.
...
changes can be reviewed on a live URL before they touch the real site. Mirrors the
`chessscenes-rebuild` / `zoosnap-staging` pattern used in this user's other projects, one
Railway service ≈ one deploy target.
...
- **Domain**: `https://library-staging.up.railway.app` — renamed 2026-09-23 from the
  auto-generated `chess-library-api-staging-production.up.railway.app` (`railway domain
  update <old-domain> --domain library-staging --service chess-library-api-staging`),
  which was both absurdly long and, worse, exposed the shared-database sharp edge
  described below right there in the URL bar every time someone typed it out by hand.
- **Has its own dedicated Postgres, separate from prod's** (service `Postgres-wpZy`, id
  `61107523-8529-4519-a891-7b3f5eb3ac80`, `DATABASE_URL` set to the reference
  `${{Postgres-wpZy.DATABASE_URL}}`) — **as of 2026-09-23, replacing an earlier design
  where this service shared prod's own Postgres** (`${{Postgres.DATABASE_URL}}`), split
  only by table-name suffix (`_dev`) and a `/dev/` URL path prefix. That design had a real,
  repeatedly-flagged sharp edge: this staging service's own bare `/` path (not `/dev/`)
  read/wrote the exact same tables real users' production data lived in, so testing here
  safely meant remembering to always use the uglier `/dev/` URL instead of the service's
  own root — easy to get wrong, and the whole reason "let's simplify the URL" (dropping
  `/dev/` for a plain, memorable domain) couldn't just be a cosmetic rename. Fixed by
  provisioning `Postgres-wpZy` and pointing this service's `DATABASE_URL` at it instead;
  see `app.py`'s "Prod/dev split" comment and the root "Prod/dev split" section above for
  the resulting code simplification (no more `env` parameter, no more `_dev` tables, no
  more `/dev/`-prefixed routes anywhere). The old shared Postgres's `_dev`-suffixed tables
  were left in place, unused, rather than dropped (see "Schema" above) — this service's
  pre-migration dev data (test signups, test libraries) was treated as disposable and was
  **not** migrated into the new database; staging started fresh from an empty DB.
  `ALLOWED_ORIGIN` includes the new staging domain alongside what was copied from `web`.
- **Workflow**: branch off `main` → merge into `staging` → push → review at
  `library-staging.up.railway.app` (desktop and real mobile emulation per the frontend's
  own testing convention) → once approved, merge `staging` into `main` → push → deploys
  to the real site. `staging` should generally track `main` (fast-forward it after every
  prod promotion) so it doesn't drift into being its own permanent fork.
...
- **Prod and staging each get their own database, full stop — don't go back to a shared
  Postgres split by table suffix "to save a resource."** The original shared-Postgres
  design (see "Staging environment") existed to avoid provisioning a second database, and
  it worked, but it had a standing sharp edge (staging's own bare `/` silently touched
  real production data) that required active, ongoing discipline to avoid, rather than
  being structurally impossible. The 2026-09-23 migration traded a small amount of
  Railway resource cost for making that class of mistake unavailable rather than merely
  discouraged — worth the tradeoff, and worth repeating for any future environment split
  in this or similar projects, rather than reaching for a table-suffix/path-prefix split
  as the default.
- **A share link's own domain is derived from the request that created it
  (`request_origin()`), never hardcoded.** The previous hardcoded `SHARE_HOST` was a
  latent bug waiting for the day a second, separately-databased deployment existed (see
  "single-game sharing" above) — it just hadn't been exercised yet because staging shared
  prod's database. Don't hardcode a domain anywhere new content is generated per-request;
  derive it from `request.host`/`X-Forwarded-Proto` instead.
